...
// === SYSTEM CONSTANTS ===
    ...
    // PLL settings
        ...
        const uint8_t numXTALFreqOptions = sizeof(PLL_XTAL_OPTIONS) / sizeof(PLL_XTAL_OPTIONS[0]); // total number of selectable PLL crystal frequencies; changing this count requires EEPROM layout migration for stored band frequencies/user memories
...
void handleMenu() {
    ...
    if (menuMode && !stationNameEditMode) display(MENU_ENTRY_TITLE, false); // advance a running menu-entry animation without blocking menu/PLL handling
    ...
    const bool settingsChanged = menuMode && menuLevel <= MENU_LEVEL_GENERAL_SETTINGS && (
        ...
        strncmp(menuSettings.stationName, stationName, maxNameLength + 1) != 0);
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                    ...
                    case PLL_MENU_PORT_MAPPING:
                        ...
                        if (menuEditMode) {
                            ...
                            if (outputPortsEditPhase == OUTPUT_PORT_PHASE_LOCKED) {
                                lcd.print(F("locked: "));
                                if (isValidPLLPort(menuSettings.portIdxLock)) {
                                    lcd.print(F("P"));
                                    lcd.print(menuSettings.portIdxLock);
                                } else {
                                    lcd.print(F("none"));
                                }
                            } else if (outputPortsEditPhase == OUTPUT_PORT_PHASE_UNLOCKED) {
                                lcd.print(F("unlocked: "));
                                if (isValidPLLPort(menuSettings.portIdxUnlock)) {
                                    lcd.print(F("P"));
                                    lcd.print(menuSettings.portIdxUnlock);
                                } else {
                                    lcd.print(F("none"));
                                }
                            } else {
                                lcd.print(F("RF drive: "));
                                if (isValidPLLPort(menuSettings.portIdxRF)) {
                                    lcd.print(F("P"));
                                    lcd.print(menuSettings.portIdxRF);
                                } else {
                                    lcd.print(F("none"));
                                }
                            }
                ...
                switch (menuIndex) {
                    ...
                    case GENERAL_MENU_BACKLIGHT_DIMMER:
                        ...
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            lcd.print(menuSettings.backlightDimActive ? F("on ") : F("off"));
                        } else {
                            lcd.print(F("SET to select"));
                        }
                        break;
                    case GENERAL_MENU_SHOW_MENU_TITLE:
                        ...
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            lcd.print(menuSettings.showMenuTitle ? F("on ") : F("off"));
                        } else {
                            lcd.print(F("SET to select"));
                        }
                        break;
