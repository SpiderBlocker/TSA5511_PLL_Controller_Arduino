...
USAGE
  • Double-clicking SET opens the SYSTEM MENU. In system submenus, holding SET returns one level back while keeping pending changes in the temporary menu state
    until they are explicitly saved or discarded. In the station name editor, SET confirms characters and holding SET returns from the editor while keeping the edited
    temporary name until it is explicitly saved or discarded. The available system settings are as follows:
  ...
  • Hold UP and DOWN together for 1 second from the main interface to open the SERVICE MENU. A brief chord-acquisition window delays the first individual UP/DOWN action
    just long enough to recognize the combination without briefly entering frequency edit mode. Alternatively, hold SET through the startup splash screen to enter the
    SERVICE MENU before normal PLL initialization. The SERVICE MENU provides service information and maintenance functions:
  ...
  • During normal operation, I²C communication loss is indicated and retried automatically. Any active menu is closed and any unconfirmed frequency edit is cancelled,
    except that DIAGNOSTICS retains the selected page during recovery; a fixed "!" at the upper-right marks diagnostic data pages as temporarily stale while the fault
    indicator blinks (the RETURN TO MENU navigation page is intentionally left unmarked). Pressing any button briefly shows the I²C error/retry message without performing
    an action, then returns to the frozen diagnostics page. Any pending settings are still discarded for safety. After communication is restored, the PLL is fully
    reprogrammed only if a write may have failed or the TSA5511 POR flag indicates an unexpected reset. After a read-only interruption without POR, the existing
    programming is retained; if lock was lost, acquisition control and lock verification are resumed.
*/
...
// === SYSTEM CONSTANTS ===
    ...
    const unsigned long i2cErrorFeedbackDelay = 1000; // temporary I²C error message after user input during frozen diagnostics
...
// === RUNTIME STATE VARIABLES ===
    ...
    uint8_t i2cRecoveryCount = 0; // successfully completed I²C recoveries since controller startup (saturates at 100, displayed as 99+)
    ...
    unsigned long lockTime = 0; // acquisition start timestamp while pending; last successful lock duration otherwise
    ...
    uint8_t porCount = 0; // unexpected POR events observed since controller startup, excluding the initial POR baseline (saturates at 100, displayed as 99+)
...
// === FUNCTION PROTOTYPES ===
    ...
    void display(uint8_t mode, bool refresh = true, bool allowDuringRecovery = false);
...
void initialize() {
    ...
    // hold SET through the startup splash screen to enter SERVICE MENU before normal PLL initialization
    if (!digitalRead(setButton)) {
        ...
        factoryResetConfirm = factoryResetFinalConfirm = false;
    ...
    if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &pllStatusByte, 1)) {
        if (pllStatusValid && (pllStatusByte & PLL_BYTE1_POR) && porCount < 100) porCount++;
        pllStatusValid = true; // read and retain the pre-programming status byte
    }
    configurePLL(false);
...
void handleFrequencyChange() {
    ...
    static int8_t servicePress = 0; // -1/+1 = pending DOWN/UP; 2 = accepted chord; 3 = cancelled chord awaiting full release

    if (menuMode || i2cRecoveryState) {
        servicePress = 0;
        return;
    }
    unsigned long currentMillis = millis();
    ...
    if (!freqSetMode && !backlightOff) {
        ...
        if (servicePress >= 2) {
            if (servicePress == 2 && downHeld && upHeld && setReleased) {
            ...
            servicePress = 3; // interrupted chord: suppress residual UP/DOWN until both buttons are fully released
        ...
        if (setReleased && (downHeld != upHeld)) {
            ...
            servicePress = 0;
            applyFrequencyChange(true, &targetFreq, direction);
            freqSetInactivityTimer = currentMillis;
            return;
            ...
            servicePress = 0;
            if (shortPress) { // preserve very short UP/DOWN taps that end inside the chord-acquisition window
                applyFrequencyChange(true, &targetFreq, direction);
                freqSetInactivityTimer = currentMillis;
                return;
            }
    ...
    if (freqSetMode && currentMillis - freqSetInactivityTimer > freqSetTimeout) {
        freqSetMode = false;
        servicePress = (!digitalRead(downButton) || !digitalRead(upButton)) ? 3 : 0;
        readSetGesture(true);
        targetFreq = currentFreq;
        display(MAIN_INTERFACE);
    }
...
void handleMenu() {
    ...
    static unsigned long i2cErrorFeedbackTime = 0;
    static bool i2cErrorFeedbackButtonDown = false;
    unsigned long currentMillis = millis();

    if (i2cRecoveryState) {
        const bool buttonActive = buttonDownState || buttonSetState || buttonUpState;
        if (menuLevel == MENU_LEVEL_DIAGNOSTICS) {
            if (buttonActive && !i2cErrorFeedbackButtonDown) {
                i2cErrorFeedbackButtonDown = true;
                i2cErrorFeedbackTime = currentMillis ? currentMillis : ~0UL;
                display(I2C_ERROR);
            } else if (!buttonActive) i2cErrorFeedbackButtonDown = false;

            if (i2cErrorFeedbackTime && currentMillis - i2cErrorFeedbackTime >= i2cErrorFeedbackDelay) {
                i2cErrorFeedbackTime = 0;
                display(MENU_INTERFACE, true, true);
                if (menuIndex != DIAGNOSTICS_MENU_EXIT) {
    ...
    i2cErrorFeedbackTime = 0;
    i2cErrorFeedbackButtonDown = false;
    ...
    const bool settingsChanged = menuMode && menuLevel <= MENU_LEVEL_GENERAL_SETTINGS && (
        tempPllAddrIndex != originalSettings.pllAddrIndex ||
        xtalFreqIndex != originalSettings.xtalFreqIndex ||
        pllControlMode != originalSettings.pllControlMode ||
        portIdxLock != originalSettings.portIdxLock ||
        portIdxUnlock != originalSettings.portIdxUnlock ||
        portIdxRF != originalSettings.portIdxRF ||
        numDecimals != originalSettings.numDecimals ||
        memcmp(freqBandIndex, originalSettings.freqBandIndex, sizeof(freqBandIndex)) != 0 ||
        backlightDimActive != originalSettings.backlightDimActive ||
        showMenuTitle != originalSettings.showMenuTitle ||
        strncmp(stationNameBuffer, originalSettings.stationName, maxNameLength + 1) != 0);
    ...
    auto returnToMainInterface = []() {
        if (startupServiceMode) {
            ...
            ignoreFirstSetInMenu = buttonSetState; // ignore a still-held SET until release
            return;
        }
        menuMode = false;
        menuEditMode = false;
    ...
    if (!menuMode) {
        ...
        if (!buttonSetState && clickStartTime != 0) {
            ...
            if (clickDuration < setClickInterval) {
                ...
                lastShortClickTime = validDoubleClick ? 0 : (currentMillis ? currentMillis : ~0UL);
                if (validDoubleClick) { // valid double-click detected: enter settings menu
                    ...
                    originalSettings.pllAddrIndex = pllAddrIndex;
                    originalSettings.xtalFreqIndex = xtalFreqIndex;
                    originalSettings.pllControlMode = pllControlMode;
                    originalSettings.portIdxLock = portIdxLock;
                    originalSettings.portIdxUnlock = portIdxUnlock;
                    originalSettings.portIdxRF = portIdxRF;
                    originalSettings.numDecimals = numDecimals;
                    memcpy(originalSettings.freqBandIndex, freqBandIndex, sizeof(freqBandIndex));
                    originalSettings.backlightDimActive = backlightDimActive;
                    originalSettings.showMenuTitle = showMenuTitle;
                    memcpy(originalSettings.stationName, stationNameBuffer, sizeof(stationNameBuffer));
                    outputPortsEditPhase = OUTPUT_PORT_PHASE_LOCKED;
                    display(MENU_ENTRY_TITLE);
                    display(MENU_INTERFACE);
    ...
    if (menuLevel == MENU_LEVEL_SERVICE_MENU) {
        if (menuEditMode) {
            ...
            // FACTORY RESET retains the existing two-stage confirmation
        ...
        if (setShortAction) {
            ...
            } else if (menuIndex == SERVICE_MENU_I2C_FALLBACK && startupServiceMode) {
                restoreI2CDefaults();
                return;
            } else {
                menuEditMode = true; // SYSTEM INFO view or FACTORY RESET confirmation
                if (menuIndex == SERVICE_MENU_FACTORY_RESET) factoryResetConfirm = factoryResetFinalConfirm = false;
            }
            display(MENU_INTERFACE);
        }
        return;
    ...
    if (menuLevel == MENU_LEVEL_DIAGNOSTICS) {
        ...
        if (menuIndex != DIAGNOSTICS_MENU_EXIT && currentMillis - lastDiagnosticsRefresh >= diagnosticsInterval) {
            lastDiagnosticsRefresh = currentMillis;
            display(MENU_INTERFACE, false); // update live values without clearing/flickering the LCD
        }
        return;
    }
        ...
        if (menuLevel == MENU_LEVEL_VCO_SETTINGS) {
            ...
            switch (menuIndex) {
                ...
                case PLL_MENU_PLL_CONTROL:
                    handleButtonRepeat(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (pllControlMode == VARICAP_DRIVE_OFF) pllControlMode = CP_LOW;
                        else if (pllControlMode == CP_LOW) pllControlMode = CP_HIGH;
                        display(MENU_INTERFACE);
                    });
                    handleButtonRepeat(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (pllControlMode == CP_HIGH) pllControlMode = CP_LOW;
                        else if (pllControlMode == CP_LOW) pllControlMode = VARICAP_DRIVE_OFF;
                        display(MENU_INTERFACE);
                    });
                    break;
...
void monitorPLL() {
    ...
    static uint8_t retryCount = 0, lockVerifyCount = 0;
    unsigned long currentMillis = millis();
    const bool diagnosticsActive = (menuLevel == MENU_LEVEL_DIAGNOSTICS); // also retained as a recovery marker while menuMode is temporarily false
    ...
    if (startupServiceMode) {
        ...
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
            if (retryCount >= i2cMaxRetries && i2cRecoveryCount < 100) i2cRecoveryCount++; // count restored communication after a sustained startup diagnostics fault
            if (pllStatusValid && (statusByte & PLL_BYTE1_POR) && porCount < 100) porCount++; // exclude the first valid status read as the expected power-on POR baseline
            if (retryCount && menuIndex != DIAGNOSTICS_MENU_EXIT) {
                lcd.setCursor(lcdColumns - 1, 0);
                lcd.print(F(" ")); // clear stale-data marker immediately after communication recovers
            }
            retryCount = 0;
    ...
    if (!attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
        ...
        if (!i2cRecoveryState) {
            ...
            if (menuMode && !diagnosticsActive) restoreSettings(); // diagnostics retains its selected page
            i2cRecoveryState = I2C_RECOVERY_READ;
            pllLock = false; // preserve pllCheckPending to retain the pre-error acquisition state
            freqSetMode = menuMode = false;
            if (diagnosticsActive) menuLevel = MENU_LEVEL_DIAGNOSTICS; // retain read-only diagnostics as recovery destination
            backlightOffRequested = backlightOff = false;
            digitalWrite(lockIndicator, LOW);
            if (diagnosticsActive) {
                if (menuIndex != DIAGNOSTICS_MENU_EXIT) {
                    lcd.setCursor(lcdColumns - 1, 0);
                    lcd.print(F("!")); // keep data page visible; values are stale until I²C recovers
                }
    ...
    retryCount = 0;
    if (pllStatusValid && (statusByte & PLL_BYTE1_POR) && porCount < 100) porCount++; // exclude the first successful status read as the power-on POR baseline
    pllStatusByte = statusByte;
    pllStatusValid = true;
    ...
    if (i2cRecoveryState || (statusByte & PLL_BYTE1_POR)) {
        ...
        if (menuMode && !diagnosticsActive) restoreSettings(); // diagnostics retains its selected page
        freqSetMode = menuMode = false;
        if (diagnosticsActive) menuLevel = MENU_LEVEL_DIAGNOSTICS;
        ...
        if (recoveryState == I2C_RECOVERY_WRITE || (statusByte & PLL_BYTE1_POR)) {
            ...
            if (pllLock && pllCheckPending) lockTime = currentMillis - lockTime;
            else if (!pllLock && !pllCheckPending) lockTime = currentMillis;
            pllCheckPending = !pllLock;
        }

        if (recoveryState && i2cRecoveryCount < 100) i2cRecoveryCount++; // count only a fully completed I²C recovery
        digitalWrite(lockIndicator, pllLock ? HIGH : LOW);
        if (diagnosticsActive) {
            buttonDownPressed = buttonDownState;
            buttonUpPressed = buttonUpState;
            ignoreFirstSetInMenu = buttonSetState;
            readSetGesture(true); // suppress any recovery-feedback key that is still held
            menuMode = true;
            menuLevel = MENU_LEVEL_DIAGNOSTICS;
    ...
    if (pllLock && updateOutputPorts(true)) {
        lockTime = currentMillis - lockTime;
...
bool attemptI2C(bool isRead, byte address, byte* buffer, byte length, bool singleAttempt) {
    ...
    if (!singleAttempt && address == PLL_ADDRESSES[pllAddrIndex]) { // exclude expected failures while probing a new address
        const bool diagnosticsActive = (menuLevel == MENU_LEVEL_DIAGNOSTICS);
        if (menuMode && !diagnosticsActive) restoreSettings(); // diagnostics retains its selected page
        if (diagnosticsActive) menuLevel = MENU_LEVEL_DIAGNOSTICS; // retain read-only diagnostics as recovery destination
        if (!isRead && length) {
            i2cRecoveryState = I2C_RECOVERY_WRITE;
            pllCheckPending = false;
        } else i2cRecoveryState = I2C_RECOVERY_READ;
        pllLock = false;
        freqSetMode = menuMode = false;
        backlightOffRequested = backlightOff = false;
        digitalWrite(lockIndicator, LOW);
...
void display(uint8_t mode, bool refresh, bool allowDuringRecovery) {
    ...
    if (i2cRecoveryState && mode != I2C_ERROR && !allowDuringRecovery) return; // preserve recovery indication until communication is restored

    // format VCO frequency exactly from integer Hz, avoiding AVR floating-point rounding artifacts
    auto formatFreq = [](char* buffer, unsigned long frequency, bool rightAlign) {
        ...
        if (rightAlign) {
            ...
            if (len < freqDisplayWidth) {
                memmove(buffer + freqDisplayWidth - len, buffer, len + 1);
                memset(buffer, ' ', freqDisplayWidth - len);
            }
        }
    };
    ...
    auto printFreq = [&](uint8_t row, unsigned long frequency, const __FlashStringHelper* prefix, const char* suffix) {
        ...
        formatFreq(buffer, frequency, true);
    ...
    switch(mode) {
        ...
        case MENU_INTERFACE:
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                switch (menuIndex) {
                    ...
                    case USER_MENU_EXIT: lcd.print(F("EXIT MENU")); break;
                }
                lcd.setCursor(0, 1);
                if (menuIndex == USER_MENU_RF_DRIVE) {
                ...
                if (menuIndex == numUserMemorySlots) {
                    ...
                    if (hasMemory) {
                        ...
                        char buffer[freqDisplayWidth + 1]; // buffer for formatted frequency string and null terminator
                        formatFreq(buffer, memoryFreq, false); // format VCO frequency as variable-width string in MHz
                ...
                switch (menuIndex) {
                    ...
                    case SERVICE_MENU_I2C_FALLBACK:
                        lcd.print(F("I2C FALLBACK"));
                        lcd.setCursor(0, 1);
                        lcd.print(F("SET to restore"));
                        break;
                ...
                switch (menuIndex) {
                    case DIAGNOSTICS_MENU_STATUS:
                        lcd.print(F("PLL STATUS"));
                        lcd.setCursor(0, 1);
                        lcd.print(F("FL:"));
                        if (pllStatusValid) lcd.print((pllStatusByte & PLL_BYTE1_FL) ? 1 : 0);
                        else lcd.print(F("-"));
                        lcd.print(F(" POR cnt:"));
                        if (porCount < 100) lcd.print(porCount);
                        else lcd.print(F("99+"));
                        break;
                    case DIAGNOSTICS_MENU_INPUTS:
                        lcd.print(F("P7 P5 P4 P6-ADC"));
                        lcd.setCursor(0, 1);
                        if (!pllStatusValid) {
                            lcd.print(F("-  -  -  ---"));
                            break;
                        ...
                        for (int8_t bit = 5; bit >= 3; bit--) {
                            lcd.print((pllStatusByte >> bit) & 1);
                            lcd.print(F("  "));
                        }
                        for (int8_t bit = 2; bit >= 0; bit--) lcd.print((pllStatusByte >> bit) & 1);
                        break;
                    case DIAGNOSTICS_MENU_LOCK_TIME: {
                        if (startupServiceMode) {
                            lcd.print(F("PLL LOCK TIME"));
                            lcd.setCursor(0, 1);
                            lcd.print(F("not initialized"));
                            break;
                        }
                        static bool displayedPending = false;
                        if (refresh) lcd.print(F("PLL LOCK TIME"));
                        ...
                        // live refresh only rewrites the seven-character time field on row 2 (columns 9..15)
                        unsigned long elapsed = pllCheckPending ? millis() - lockTime : lockTime;
                        lcd.setCursor(9, 1);
                        if (!pllCheckPending && elapsed < 10000000UL) lcd.print(F(" "));
                        if (elapsed < (pllCheckPending ? 1000000UL : 100000UL)) {
                            lcd.print(elapsed / 1000);
                            lcd.print(F("."));
                            lcd.print((elapsed % 1000) / 100);
                            lcd.print(F(" s"));
                            if (elapsed < 10000UL) lcd.print(pllCheckPending ? F("  ") : F(" "));
                            else if (pllCheckPending && elapsed < 100000UL) lcd.print(F(" "));
                        } else if (elapsed < 10000000UL) {
                            lcd.print(elapsed / 1000);
                            lcd.print(F(" s"));
                            if (pllCheckPending || elapsed < 1000000UL) lcd.print(F(" "));
                        } else lcd.print(F(">9999 s"));
                        break;
                    }
                    case DIAGNOSTICS_MENU_OUTPUT_PORTS:
                        ...
                        if (pllPortByte < 0x10) lcd.print(F("0"));
                    ...
                    case DIAGNOSTICS_MENU_I2C_RECOVERIES:
                        ...
                        lcd.setCursor(0, 1);
                        lcd.print(F("total: "));
                        if (i2cRecoveryCount < 100) lcd.print(i2cRecoveryCount);
                        else lcd.print(F("99+"));
                        break;
                ...
                switch (menuIndex) {
                    case VCO_MENU_FREQUENCY_BAND:
                        ...
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            switch (freqBandIndex[xtalFreqIndex]) {
                                case 0: lcd.print(F("FM OIRT")); break;
                                case 1: lcd.print(F("FM Japan")); break;
                                case 2: lcd.print(F("FM World")); break;
                                case 3: lcd.print(F("2 m")); break;
                                case 4: lcd.print(F("70 cm")); break;
                                case 5: lcd.print(F("UHF")); break;
                                default: lcd.print(F("64-1300 MHz")); break;
                            }
                        } else {
                            lcd.print(F("SET to select"));
                        }
                ...
                switch (menuIndex) {
                    ...
                    case PLL_MENU_PLL_CONTROL:
                        ...
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            lcd.print(pllControlMode == CP_LOW ? F("low ") : (pllControlMode == CP_HIGH ? F("high") : F("disabled")));
                        } else {
                            lcd.print(F("SET to select"));
                        }
                        break;
        ...
        case MENU_ENTRY_TITLE:
            if (showMenuTitle) {
                if (menuLevel == MENU_LEVEL_SERVICE_MENU) animateMenuEntryTitle(PSTR("SERVICE MENU"), PSTR("DIAGNOSTICS"));
                else animateMenuEntryTitle(
