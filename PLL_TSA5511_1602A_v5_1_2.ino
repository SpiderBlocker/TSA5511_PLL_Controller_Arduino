...
void display(uint8_t mode, bool refresh, bool allowDuringRecovery) {
    ...
    auto formatFreq = [](char* buffer, unsigned long frequency, bool rightAlign, uint8_t decimals) {
    ...
    auto printFreq = [&](uint8_t row, unsigned long frequency, const __FlashStringHelper* prefix, const char* suffix) {
        ...
        formatFreq(buffer, frequency, true, numDecimals);
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                ...
                if (menuIndex == numUserMemorySlots) {
                    ...
                    if (hasMemory) {
                        ...
                        char buffer[freqDisplayWidth + 1]; // buffer for formatted frequency string and null terminator
                        formatFreq(buffer, memoryFreq, false, max(numDecimals, getRequiredNumDecimals(memoryFreq))); // show exact preset without lowering current precision
