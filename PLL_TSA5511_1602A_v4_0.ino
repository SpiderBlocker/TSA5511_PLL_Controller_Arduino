...
DESCRIPTION
  ...
  It features an intuitive menu interface for making system settings and using the quick menu as explained in detail below.
...
USAGE
  • Double-clicking SET opens the SYSTEM SETTINGS menu, in which system settings can be made as follows. Holding SET within system submenus returns one level back
    while keeping pending changes in the temporary menu state until they are explicitly saved or discarded, except in the station name editor where SET is used to
    confirm characters and can be held to auto-advance through character positions:
    ...
    ■ GENERAL SETTINGS => • STATION NAME     > This sets the radio station name that is shown in quiescent condition (locked state). Select characters using UP/DOWN and
                          ...
                          • CLEAR SETTINGS   > This clears all stored settings and user memories and restores the default settings after double confirmation.
                          • RETURN           > Returns to the main settings menu.
  ...
  • Press and hold SET from the main interface to open the QUICK MENU, which provides access to frequently used end-user functions. Holding SET within the quick
    menu returns one level back or exits the quick menu from its top level:
...
// === SYSTEM CONSTANTS ===
    ...
    // PLL settings
        ...
        enum { OUTPUT_PORT_PHASE_LOCKED, OUTPUT_PORT_PHASE_UNLOCKED, OUTPUT_PORT_PHASE_RF_DRIVE, numOutputPortEditPhases }; // output port edit phase
    ...
    // timing parameters
        ...
        const unsigned long menuEntryTitleDelay = 1500; // duration to show menu entry title
        const unsigned long menuEntryAnimDelay = 90; // menu entry transition animation step delay
...
void handleMenu() {
    ...
    if (!menuEditMode) {
        ...
        if (setShortAction) {
            if (menuLevel == MENU_LEVEL_MAIN) {
                ...
                if (menuIndex == menuLength[menuLevel] - 1) {
                    ...
                    if (menuLevel == MENU_LEVEL_GENERAL_SETTINGS && menuIndex == GENERAL_MENU_CLEAR_SETTINGS) {
                        clearSettingsConfirm = false;
                        clearSettingsFinalConfirm = false;
                    }
                    menuEditMode = true;
        ...
        if (menuLevel == MENU_LEVEL_VCO_SETTINGS) {
            ...
            switch (menuIndex) {
                case GENERAL_MENU_BACKLIGHT_DIMMER:
                    ...
                    break;
                case GENERAL_MENU_CLEAR_SETTINGS:
                    handleButtonInput(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        clearSettingsConfirm = false;
                        display(MENU_INTERFACE);
                    });
        ...
        if (setShortAction) {
            ...
            } else if (menuLevel == MENU_LEVEL_GENERAL_SETTINGS && menuIndex == GENERAL_MENU_CLEAR_SETTINGS) {
...
// clear entire EEPROM and reload defaults
void clearSettings() {
