...
void readLastFrequencyBand() {
    ...
    long storedFreq = readLastBandFrequency(currentBandIndex, xtalFreqIndex);
...
void readOutputPorts() {
    uint8_t b0 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR); // read locked index (port 0..7) or 0xFF (none)
    uint8_t b1 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + 1); // read unlocked index (port 0..7) or 0xFF (none)
...
void storeBandFrequency(byte bandIndex, byte xtalIndex, long frequency) {
            ...
            if (menuLevel == 0) {
                ...
                switch (menuIndex) {
                    case 0:
                        ...
                        lcd.setCursor(0, 1);
                        if (menuEditMode) {
                            long low = validateFreq(freqBands[freqBandIndex[xtalFreqIndex]][0], true);
                            long high = validateFreq(freqBands[freqBandIndex[xtalFreqIndex]][1], true);
                            lcd.print(F("> "));
