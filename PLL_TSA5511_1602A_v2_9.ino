...
USAGE
  ...
  • If enabled, the LCD backlight will dim after a preset period in quiescent state (locked). Press and hold SET to turn off the backlight completely.
...
// === SYSTEM CONSTANTS ===
    ...
    const unsigned long PLL_XTAL_OPTIONS[] = { 1600000, 3200000 }; // PLL crystal frequency options (Hz)
...
// === RUNTIME STATE VARIABLES ===
    ...
    uint8_t portIdxLock = 5; // default output port for an external lock indicator
    uint8_t portIdxUnlock = 6; // default output port for an external unlock indicator
    uint8_t portIdxRF = 2; // default output port to activate RF output stage in locked state
    uint8_t cpMode = CP_HIGH; // default = high
    ...
    unsigned long currentFreq; // operational VCO frequency
    unsigned long targetFreq; // target VCO frequency
    byte freqBandIndex[2]; // currently selected VCO frequency band index per XTAL frequency
    ...
    uint8_t menuLevel = 0; // 0 = main menu, 1 = VCO settings, 2 = PLL settings, 3 = general settings
    uint8_t menuIndex = 0; // selected item index within current menu level
    uint8_t outputPortsEditPhase = 0; // 0 = "locked", 1 = "unlocked", 2 = "RF drive"
...
// === FREQUENCY UTILITIES ===
    // PLL reference frequency in Hz (equals minimum VCO frequency step size)
    ...
    uint8_t getStepSizeMultiplier() {
        unsigned long displayStepSize = 1000000UL;  // base display step size in Hz before applying decimal precision
        for (uint8_t i = constrain(numDecimals, 0, 3); i; i--) displayStepSize /= 10;
        return (displayStepSize + getPLLRefFreq() - 1) / getPLLRefFreq(); // multiplier to ensure that display precision is met (integer ceiling)
    }

    // VCO frequency validation
    unsigned long validateFreq(unsigned long frequency, bool alignToStepSize = false) {
        ...
        return ((frequency + step / 2) / step) * step; // align frequency to nearest valid VCO step using integer rounding
    }
...
void readNumDecimals() {
    ...
    if (val <= 3) {
        numDecimals = constrain(val, 0, (xtalFreqIndex == 0 ? 3 : 2)); // limit precision to valid range for current XTAL setting
    }
}
...
void readLastFrequencyBand() {
    ...
    lowerFreq = validateFreq(freqBands[currentBandIndex][0], true);
    upperFreq = validateFreq(freqBands[currentBandIndex][1], true);
...
void readChargePump() {
    ...
    cpMode = (v <= CP_DISABLED) ? v : CP_HIGH; // validate and apply (default = high)
}

void readOutputPorts() {
    uint8_t b0 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR); // read locked index (port 0..7), PLL_PORT_NONE, or 0xFF if EEPROM is blank
    uint8_t b1 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + 1); // read unlocked index (port 0..7), PLL_PORT_NONE, or 0xFF if EEPROM is blank
    uint8_t b2 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + 2); // read RF drive index (port 0..7), PLL_PORT_NONE, or 0xFF if EEPROM is blank
    ...
    // clamp to valid range (0..7), else 'none'
    portIdxLock = (b0 <= 7) ? b0 : PLL_PORT_NONE; // locked index (0..7) or 'none'
    portIdxUnlock = (b1 <= 7) ? b1 : PLL_PORT_NONE; // unlocked index (0..7) or 'none'
    portIdxRF = (b2 <= 7) ? b2 : PLL_PORT_NONE; // RF drive index (0..7) or 'none'
}
...
void applyFrequencyChange(bool adjusting, unsigned long* targetFreq, int8_t direction) {
    if (adjusting) {
        // UP/DOWN action
        if (freqSetMode) {
            unsigned long step = min(getStepSizeMultiplier() * getPLLRefFreq(), upperFreq - lowerFreq); // constrain step size within range
...
void handleMenu() {
    ...
    if (menuExitConfirmMode) {
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuIndex == 0) { // save changes
                ...
                restoreSettings();
                display(PLL_LOCK_STATUS);
            } else if (menuIndex == 2) { // cancel and return to the main menu
                menuExitConfirmMode = false;
                menuIndex = 0; // return to the first main menu option
                display(MENU_INTERFACE);
            }
        });
        return;
    ...
    if (!menuEditMode) {
        ...
        if (menuLevel == 1) {
            ...
            switch (menuIndex) {
                ...
                case 2:
                    ...
                    break;
                case 3: // output ports: function → output port mapping (locked → unlocked → RF drive)
                    handleButtonInput(buttonDownState, buttonDownPressed, -1, [](int8_t) {
        ...
        // SET confirms and advances phase or returns to menu navigation mode
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuLevel == 2 && menuIndex == 3) {
                // advance edit phase: locked -> unlocked -> RF drive -> exit edit mode
                if (outputPortsEditPhase < 2) {
                    outputPortsEditPhase++;
                    display(MENU_INTERFACE);
                    return; // stay in edit mode
                } else {
                    outputPortsEditPhase = 0; // reset for next time
                }
            }
            menuEditMode = false;
            display(MENU_INTERFACE);
...
// restore all user settings from EEPROM, realign target frequency and return to main interface
...
void configurePLL() {
    ...
    unsigned long divisor = (targetFreq / getPLLRefFreq()); // calculate divisor
    ...
    data[2] = (PLL_BYTE4_BASE | PLL_BYTE4_CP) // set charge pump high during acquisition, unless set to disabled
        | ((cpMode == CP_DISABLED) ? PLL_BYTE4_OS : 0);
    data[3] = (portIdxUnlock <= 7) ? _BV(portIdxUnlock) : 0; // assert unlocked if assigned; otherwise all ports low
...
void checkPLL() {
    ...
    if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &readByte, 1)) {
        ...
        if (pllLock) {
            // locked: set locked and RF drive ports high (if assigned)
            ...
            // unlocked: set unlocked port high (if assigned)
...
void storeBandFrequency(byte bandIndex, byte xtalIndex, unsigned long frequency) {
    if (bandIndex < numFreqBands && xtalIndex <= 1 && frequency != currentFreq) { // avoid unnecessary EEPROM access
        EEPROM.put(EEPROM_BAND_FREQ_BASE_ADDR + (bandIndex * 8) + (xtalIndex * 4), frequency); // calculate EEPROM address: base + (bandIndex * 8) + (xtalIndex * 4)
    }
}
    ...
    switch(mode) {
        ...
        case MENU_INTERFACE:
            ...
            if (menuExitConfirmMode) {
                ...
                lcd.print(F("> "));
                switch (menuIndex) {
                    case 0: lcd.print(F("save changes")); break;
                    case 1: lcd.print(F("discard")); break;
                    case 2: lcd.print(F("cancel")); break;
                }
                break;
            }
            if (menuLevel == 0) {
                ...
                switch (menuIndex) {
                    case 0:
                        ...
                        lcd.setCursor(0, 1);
                        if (menuEditMode) {
                            unsigned long low = validateFreq(freqBands[freqBandIndex[xtalFreqIndex]][0], true);
                            unsigned long high = validateFreq(freqBands[freqBandIndex[xtalFreqIndex]][1], true);
                            lcd.print(F("> "));
                ...
                switch (menuIndex) {
                    ...
                    case 1:
                        ...
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            lcd.print(PLL_XTAL_OPTIONS[xtalFreqIndex] / 1000000UL);
                            lcd.print(F("."));
                            lcd.print((PLL_XTAL_OPTIONS[xtalFreqIndex] % 1000000UL) / 100000UL);
                            lcd.print(F(" MHz"));
                        } else {
                            lcd.print(F("SET to select"));
                        }
                    ...
                    case 3:
                        ...
                        if (menuEditMode) {
                            ...
                            if (outputPortsEditPhase == 0) {
                                lcd.print(F("locked: "));
                                if (portIdxLock <= 7) { lcd.print(F("P")); lcd.print(portIdxLock); } else { lcd.print(F("none")); }
                            } else if (outputPortsEditPhase == 1) {
                                lcd.print(F("unlocked: "));
                                if (portIdxUnlock <= 7) { lcd.print(F("P")); lcd.print(portIdxUnlock); } else { lcd.print(F("none")); }
                            } else {
                                lcd.print(F("RF drive: "));
                                if (portIdxRF <= 7) { lcd.print(F("P")); lcd.print(portIdxRF); } else { lcd.print(F("none")); }
                            }
                        } else {
                            lcd.print(F("SET to select"));
                        }
