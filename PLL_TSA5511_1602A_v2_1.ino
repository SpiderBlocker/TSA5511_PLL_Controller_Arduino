...
// PLL settings
const byte pllAddresses[] = { 0x60, 0x61, 0x62, 0x63 }; // depends on TSA5511 P3 configuration; refer to TSA511 datasheet table 4 (address 0x61 is always valid)
uint8_t pllAddrIndex = 1; // default index (address 0x61)
uint8_t tempPllAddrIndex = pllAddrIndex; // temporary index used for address selection in menu edit mode
...
void i2cSafeDefault() {
    if (!digitalRead(setButton)) { // SET pressed at startup
        pllAddrIndex = 1; // I²C address safe default fallback
        PLL_ADDR = pllAddresses[pllAddrIndex];
        storeI2CAddress();
...
void readOutputPortsSetting() {
    ...
    if (val <= 3) {
        ...
        pllOutputPorts = 3; // default to P2/P5 high
    }
}
        ...
        if (!buttonSetState && clickStartTime != 0) {
            ...
            if (clickDuration < setClickInterval) {
                ...
                if (validDoubleClick) { // valid double-click detected: enter main menu
                    ...
                    tempPllAddrIndex = pllAddrIndex; // initialize temporary I2C address index
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuIndex == 0) { // save changes
                ...
                if (tempPllAddrIndex != prevPllAddrIndex) {
                    if (attemptI2C(false, pllAddresses[tempPllAddrIndex], nullptr, 0)) { // new I²C address is valid
            ...
            if (menuLevel == 0) {
                switch (menuIndex) {
                    ...
                    case 2: // start station name editor
                        ...
                        return;
                    case 3: menuEditMode = true; break; // backlight dimmer setting
                    case 4: menuExitConfirmMode = true; menuIndex = 0; break; // enter exit menu
                }
                display(MENU_INTERFACE);
            } else {
                if ((menuLevel == 1 && menuIndex == 2) || (menuLevel == 2 && menuIndex == 4)) {
...
void configurePLL() {
    ...
    data[3] = PLL_ALL_LOW;
    if (attemptI2C(false, PLL_ADDR, data, 4)) { // I²C transmission with i2cMaxRetries
...
void checkPLL() {
    ...
    byte readByte;
    if (attemptI2C(true, PLL_ADDR, &readByte, 1)) {
        ...
        if (pllLock) {
            ...
            if (attemptI2C(false, PLL_ADDR, data, 2)) { // I²C transmission with i2cMaxRetries
            ...
            if (menuLevel == 0) {
                ...
                switch (menuIndex) {
                    case 0:
                        ...
                        if (menuEditMode) {
                            lcd.print("> 0x");
                            lcd.print(pllAddresses[tempPllAddrIndex], HEX);
                        } else {
                            lcd.print("SET to edit");
                        }
                        break;
