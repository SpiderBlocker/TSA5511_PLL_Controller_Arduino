  ...
  • Double-clicking SET opens the main configuration menu, in which system settings can be made as follows:
    ...
    ■ VCO SETTINGS     => • FREQUENCY BAND   > Various predefined frequency bands are available for selection. The last operating frequency will be stored in EEPROM for
                          ...
                          • PRECISION        > This sets the decimal precision at which the VCO frequency can be set and will be displayed. Note that if it is set to a
                                               ...
                                               changed to a lower value if so desired. Refer to additional explanation below at PLL SUBMENU > XTAL FREQ.
...
// === SYSTEM CONSTANTS ===
    ...
    // menu
    const uint8_t menuLength[] = { 4, 3, 5, 4 }; // number of selectable menu items per level: 0 = main, 1 = VCO SETTINGS, 2 = PLL SETTINGS, 3 = GENERAL SETTINGS
...
// === RUNTIME STATE VARIABLES ===
    ...
    bool clearSettingsConfirm = false; // true when clear settings confirmation is set to yes
    bool clearSettingsFinalConfirm = false; // true when final clear settings confirmation is active
...
void readOutputPorts() {
    uint8_t b0 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR); // read locked index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank
    uint8_t b1 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + 1); // read unlocked index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank
    uint8_t b2 = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + 2); // read RF drive index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank
...
void handleMenu() {
        ...
        if (!buttonSetState && clickStartTime != 0) {
            ...
            if (clickDuration < setClickInterval) {
                ...
                if (validDoubleClick) { // valid double-click detected: enter main menu
                    ...
                    stationNameEditMode = false;
                    clearSettingsConfirm = false;
                    clearSettingsFinalConfirm = false;
    ...
    if (!menuEditMode) {
                ...
                case 3: // output ports: function → output port mapping (locked → unlocked → RF drive)
                    handleButtonInput(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (outputPortsEditPhase == 0) {
                            if (portIdxLock == PLL_PORT_NONE) portIdxLock = (PLL_PORT_NONE - 1);
                            else if (portIdxLock == 0) portIdxLock = PLL_PORT_NONE;
                            else portIdxLock--;
                        } else if (outputPortsEditPhase == 1) {
                            if (portIdxUnlock == PLL_PORT_NONE) portIdxUnlock = (PLL_PORT_NONE - 1);
                            else if (portIdxUnlock == 0) portIdxUnlock = PLL_PORT_NONE;
                            else portIdxUnlock--;
                        } else {
                            if (portIdxRF == PLL_PORT_NONE) portIdxRF = (PLL_PORT_NONE - 1);
                    ...
                    handleButtonInput(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (outputPortsEditPhase == 0) {
                            ...
                            else if (portIdxLock == (PLL_PORT_NONE - 1)) portIdxLock = PLL_PORT_NONE;
                            else portIdxLock++;
                        } else if (outputPortsEditPhase == 1) {
                            if (portIdxUnlock == PLL_PORT_NONE) portIdxUnlock = 0;
                            else if (portIdxUnlock == (PLL_PORT_NONE - 1)) portIdxUnlock = PLL_PORT_NONE;
                            else portIdxUnlock++;
                        } else {
                            if (portIdxRF == PLL_PORT_NONE) portIdxRF = 0;
                            else if (portIdxRF == (PLL_PORT_NONE - 1)) portIdxRF = PLL_PORT_NONE;
                            else portIdxRF++;
                        }
                        display(MENU_INTERFACE);
                    });
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            ...
            } else if (menuLevel == 3 && menuIndex == 2) {
                if (clearSettingsConfirm) {
                    if (clearSettingsFinalConfirm) {
                        clearSettings();
                        menuUnlockTime = millis() + setClickInterval;
                        ignoreFirstSetInMenu = true;
                        return;
                    }
                    clearSettingsFinalConfirm = true;
                    clearSettingsConfirm = false;
                    display(MENU_INTERFACE);
                    return;
                }
...
// clear all EEPROM locations used by this sketch and reload defaults
void clearSettings() {
    uint16_t settingsEndAddr = EEPROM_BAND_FREQ_BASE_ADDR + (numFreqBands * numXTALFreqOptions * sizeof(unsigned long));
    for (uint16_t addr = EEPROM_NAME_ADDR; addr < settingsEndAddr; addr++) {
...
void checkPLL() {
    ...
    if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &readByte, 1)) {
        ...
        if (pllLock) {
            ...
            if (isValidPLLPort(portIdxRF)) portsHigh |= (1 << portIdxRF);
            ...
            if (isValidPLLPort(portIdxUnlock)) portsHigh |= (1 << portIdxUnlock);
        }
            ...
            if (menuLevel == 0) {
                ...
                switch (menuIndex) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        lcd.print(clearSettingsFinalConfirm ? F("CLEAR ALL DATA?") : F("CLEAR SETTINGS"));
                        lcd.setCursor(0, 1);
                        if (menuEditMode) {
                            lcd.print(F("> "));
                            lcd.print(clearSettingsConfirm ? F("yes") : F("no "));
                        } else {
                            lcd.print(F("SET to select"));
                        }
                        break;
