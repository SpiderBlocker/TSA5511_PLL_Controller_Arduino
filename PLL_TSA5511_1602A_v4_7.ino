...
USAGE
  ...
  • PLL lock is verified after programming. To prevent false unlock indications caused by FM modulation, lock-flag polling is intentionally stopped after lock has been
    detected; periodic TSA5511 status monitoring nevertheless remains active, including during menu operation and frequency editing.
  ...
  • I²C communication loss is indicated and retried automatically. Any active menu is closed with unsaved changes discarded and any unconfirmed frequency edit is
    cancelled. After communication is restored, the PLL is fully reprogrammed only if a write may have failed or the TSA5511 POR flag indicates an unexpected reset.
    After a read-only interruption without POR, the existing programming is retained; if lock was lost, acquisition control and lock verification are resumed.
*/
...
void handleFrequencyChange() {
    ...
    if (menuMode || i2cRecoveryState) return;
...
void monitorPLL() {
    ...
    const unsigned long checkInterval =
        ...
        (i2cRecoveryState || !pllCheckPending) ? i2cHealthCheckInterval : 0;
    ...
    if (i2cRecoveryState || (statusByte & PLL_BYTE1_POR)) {
        ...
        if (menuMode) restoreSettings(); // discard unsaved menu changes before recovery handling
        freqSetMode = menuMode = false;
...
void display(uint8_t mode) {
    if (i2cRecoveryState && mode != I2C_ERROR) return; // preserve recovery indication until communication is restored
