...
USAGE
  ...
  • Hold UP and DOWN together for 1 second from the main interface to open the SERVICE MENU. A brief chord-acquisition window delays the first individual UP/DOWN action
    just long enough to recognize the combination without briefly entering frequency-set mode. The SERVICE MENU provides service information and maintenance functions:
    ...
    ■ SERVICE MENU     => • DIAGNOSTICS      > Shows live TSA5511 lock/input status, live/completed PLL lock acquisition timing, unexpected POR and successfully completed
                                               I²C recovery counts since initialization, and the last successfully programmed output-port bitmap. Use UP/DOWN to browse
                                               the read-only pages; select RETURN TO MENU or hold SET to return to the SERVICE MENU.
  ...
  • The SYSTEM MENU will time out after a preset period of inactivity, discarding any unsaved changes and returning to the main screen. The save/discard/cancel exit menu
    requires explicit user action. The SERVICE MENU and DIAGNOSTICS use a longer inactivity timeout before returning to the main screen.
  ...
  • Change VCO frequency using UP/DOWN and confirm with a short SET press. Holding SET cancels the frequency change and returns to the main screen unchanged.
  ...
  • I²C communication loss is indicated and retried automatically. Any active menu is closed and any unconfirmed frequency edit is cancelled, except that DIAGNOSTICS
    retains the selected page during recovery; a fixed "!" at the upper-right marks the displayed values as temporarily stale while the fault indicator blinks. Pressing
    any button briefly shows the I²C error/retry message without performing an action, then returns to the frozen diagnostics page. Any pending settings are still
    discarded for safety. After communication is restored, the PLL is fully reprogrammed only if a write may have failed or the TSA5511 POR flag indicates an unexpected
    reset. After a read-only interruption without POR, the existing programming is retained; if lock was lost, acquisition control and
    lock verification are resumed.
*/
...
// === RUNTIME STATE VARIABLES ===
    ...
    uint8_t i2cRecoveryCount = 0; // successfully completed I²C recoveries since initialization (saturates at 100, displayed as 99+)
    ...
    uint8_t porCount = 0; // unexpected POR events observed after initialization (saturates at 100, displayed as 99+)
...
void initialize() {
    ...
    restoreI2CDefaults();
    readSettings();
    attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &pllStatusByte, 1); // clear and retain the initial POR/status byte
    configurePLL(false);
...
void handleMenu() {
    ...
    // discard changes and exit menu on timeout if inactive (excluding exit menu; service/diagnostics use extended timeout)
...
void monitorPLL() {
    ...
    if (!attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
        ...
        if (!i2cRecoveryState) {
            ...
            if (diagnosticsActive) {
                lcd.setCursor(lcdColumns - 1, 0);
                lcd.print(F("!")); // keep page visible; values are stale until I2C recovers
    ...
    if ((statusByte & PLL_BYTE1_POR) && porCount < 100) porCount++;
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                ...
                switch (menuIndex) {
                    case DIAGNOSTICS_MENU_STATUS:
                        ...
                        lcd.setCursor(0, 1);
                        lcd.print(F("FL:"));
                        lcd.print((pllStatusByte & PLL_BYTE1_FL) ? 1 : 0);
                        lcd.print(F(" POR cnt:"));
