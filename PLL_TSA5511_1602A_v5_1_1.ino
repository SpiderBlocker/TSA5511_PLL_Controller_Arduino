...
HARDWARE
  ...
  • Pull-up resistors on SDA/SCL are required. Especially if SDA/SCL ru ns through RF-decoupling circuitry, you may want to use lower values for reliable communication,
    such as 1 or 2 kΩ.
...
void monitorPLL() {
    ...
    if (startupServiceMode) {
        ...
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
            if (retryCount >= i2cMaxRetries && i2cRecoveryCount < diagnosticCountSaturation) i2cRecoveryCount++; // count restored communication after a sustained startup diagnostics fault
            if (pllStatusValid && (statusByte & PLL_BYTE1_POR) && porCount < diagnosticCountSaturation) porCount++; // exclude the first valid status read as the expected power-on POR baseline
    ...
    retryCount = 0;
    if (pllStatusValid && (statusByte & PLL_BYTE1_POR) && porCount < diagnosticCountSaturation) porCount++; // exclude the first successful status read as the power-on POR baseline
    pllStatusByte = statusByte;
    pllStatusValid = true;
