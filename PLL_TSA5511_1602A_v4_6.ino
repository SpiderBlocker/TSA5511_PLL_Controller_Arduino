...
USAGE
  • Double-clicking SET opens the SYSTEM SETTINGS menu. In system submenus, holding SET returns one level back while keeping pending changes in the temporary menu state
    ...
    ■ PLL SETTINGS     => • I2C ADDRESS      > This allows selecting the appropriate I²C address based on the actual hardware configuration of the TSA5511.
                                               By applying a DC bias to pin P3 of the TSA5511, the I²C address can be configured to 0x60, 0x62, or 0x63, while 0x61 is always valid
    ...
    ■ GENERAL SETTINGS => • STATION NAME     > This sets the radio station name that is shown in the idle locked state. Select characters using UP/DOWN and
                                               confirm each character with SET. Hold UP/DOWN to auto-scroll characters; hold SET to return from the editor.
  ...
  • The SYSTEM SETTINGS menu will time out after a preset period of inactivity, discarding any unsaved changes and returning to the main screen — except when the
  ...
  • PLL lock is verified after programming. To prevent false unlock indications caused by FM modulation, lock-flag polling is intentionally stopped after lock
    has been detected; periodic TSA5511 status monitoring nevertheless remains active, including during menu operation and frequency editing.
  ...
  • I²C communication loss is indicated and retried automatically. If recovery is required, or the TSA5511 POR flag indicates an unexpected reset, any active menu
    is closed with unsaved changes discarded and any unconfirmed frequency edit is cancelled; the PLL is then fully reprogrammed without user interaction.
*/
...
// === RUNTIME STATE VARIABLES ===
    ...
    bool i2cRecoveryActive = false; // true while automatically retrying lost I²C communication
...
// === PLL OUTPUT PORT UTILITIES ===
    ...
    bool updateLockedOutputPorts() {
        ...
        data[0] = PLL_BYTE4_BASE // set locked-state PLL control
            | ((pllControlMode == CP_HIGH) ? PLL_BYTE4_CP : 0)
...
void initialize() {
    ...
    restoreI2CDefaults();
    readSettings();
    attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1); // clear initial POR flag before monitoring for unexpected TSA5511 resets
    configurePLL(false);
...
void handleFrequencyChange() {
    ...
    if (menuMode || i2cRecoveryActive) return;
    ...
    if (menuExitConfirmMode) {
        ...
        if (setShortAction) {
            if (menuIndex == EXIT_MENU_SAVE_CHANGES) { // save changes
                ...
                readLastFrequencyBand();
                configurePLL(false); // update PLL configuration
                restoreSettings();
...
void configurePLL(bool force) {
    ...
    if (targetFreq == currentFreq && !menuMode && !force) return;
...
void monitorPLL() {
    ...
    unsigned long currentMillis = millis();
    if (i2cRecoveryActive) blinkLed(errIndicator, errBlinkRate);

    const unsigned long checkInterval = retryCount ? i2cRetryDelay : (pllCheckPending ? 0 : i2cHealthCheckInterval);
    ...
    if (!attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
        ...
        if (!i2cRecoveryActive) {
            ...
            if (menuMode) restoreSettings(); // discard unsaved menu changes before recovery
            i2cRecoveryActive = true;
            pllLock = pllCheckPending = false;
    ...
    retryCount = 0;
    if (i2cRecoveryActive || (statusByte & PLL_BYTE1_POR)) {
        if (freqSetMode) targetFreq = currentFreq; // discard unconfirmed frequency change before reprogramming
        if (menuMode) restoreSettings(); // discard unsaved menu changes before reprogramming
        freqSetMode = menuMode = false;
        i2cRecoveryActive = false;
        pllLock = false;
        digitalWrite(lockIndicator, LOW);
        digitalWrite(errIndicator, LOW);
        configurePLL(true); // restore PLL configuration after lost communication or an unexpected TSA5511 reset
        display(MAIN_INTERFACE);
        display(PLL_LOCK_STATUS);
        return;
    }
...
void display(uint8_t mode) {
    if (i2cRecoveryActive && mode != I2C_ERROR) return; // preserve recovery indication until communication is restored
