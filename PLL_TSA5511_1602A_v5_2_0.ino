...
USAGE
  ...
  • During normal operation, I²C communication loss is indicated and retried automatically. System/Quick menus are closed and any unconfirmed frequency edit is cancelled,
    ...
    data pages except I2C ADDRESS, I2C RECOVERIES and SYSTEM UPTIME; SYSTEM UPTIME continues to refresh during recovery. Live FL/input fields show "-" and PLL lock time
    shows "<unknown>" (RETURN TO MENU remains unmarked).
    A read-only interruption also
...
void monitorPLL() {
    ...
    if (startupServiceMode) {
        ...
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
            ...
            if (retryCount && menuIndex != DIAGNOSTICS_MENU_I2C_ADDRESS && menuIndex != DIAGNOSTICS_MENU_I2C_RECOVERIES &&
                menuIndex != DIAGNOSTICS_MENU_UPTIME && menuIndex != DIAGNOSTICS_MENU_EXIT)
            ...
            if (menuIndex != DIAGNOSTICS_MENU_I2C_ADDRESS && menuIndex != DIAGNOSTICS_MENU_I2C_RECOVERIES &&
                menuIndex != DIAGNOSTICS_MENU_UPTIME && menuIndex != DIAGNOSTICS_MENU_EXIT) display(I2C_STATUS_MARKER);
        }
        return;
    }
    ...
    switch(mode) {
        ...
        case MENU_INTERFACE:
            ...
            if (i2cRecoveryState && menuLevel == MENU_LEVEL_DIAGNOSTICS &&
                menuIndex != DIAGNOSTICS_MENU_I2C_ADDRESS && menuIndex != DIAGNOSTICS_MENU_I2C_RECOVERIES &&
