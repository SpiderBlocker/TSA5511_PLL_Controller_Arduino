...
# Usage
- Double-clicking SET opens the SYSTEM SETTINGS menu. In system submenus, holding SET returns one level back while keeping pending changes in the temporary menu state until they are explicitly saved or discarded. The only exception is the station name editor, where SET is used to confirm characters and can be held to auto-advance through character positions. The available system settings are as follows:
