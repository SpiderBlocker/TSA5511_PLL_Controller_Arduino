...
HARDWARE
  • The hardware comprises an ATmega328P-based 5 V Arduino Nano or a fully compatible board, a standard 16x2 LCD display (used in 4-bit mode) with backlight and
    contrast adjustment, three pushbuttons (DOWN/SET/UP, each with a 470 nF debouncing capacitor across its contact) and an optional PLL lock LED which also acts as
    a blinking fault indicator.
  ...
  • Pull-up resistors on SDA/SCL are required. Especially if SDA/SCL runs through RF-decoupling circuitry, you may want to use lower values for reliable communication,
    such as 2.2 kΩ as shown in the supplied wiring diagram.
...
USAGE
  • Double-clicking SET opens the SYSTEM MENU. System submenu settings show "SET to edit" while browsing; ordinary edit fields show the selected value prefixed with "> ",
    ...
    ■ PLL SETTINGS     => • I2C ADDRESS      > This allows selecting the appropriate I²C address based on the actual hardware configuration of the TSA5511.
                                               ...
                                               stored and cannot be reconfigured through the menu, enter the SERVICE MENU with UP+DOWN during the resulting I²C recovery,
                                               then select and confirm I2C FALLBACK to restore the default fail-safe I²C address (0x61).
  ...
  • Hold UP and DOWN together for 1 second from the main interface to open the SERVICE MENU. A brief chord-acquisition window delays the first individual UP/DOWN action
    ...
    ■ SERVICE MENU     => • DIAGNOSTICS      > Shows live TSA5511 lock/input status, live/completed PLL lock acquisition timing, PLL divisor and unexpected POR count.
                                               ...
                                               the SERVICE MENU. INPUTS shows the raw digital states of P7, P5 and P4 and the three-bit ADC code of P6. Actively driven
                                               ports are masked as '-'/'---'; high-impedance ports show their external input levels.
                          • SYSTEM INFO      > Shows the full firmware version and copyright information.
                          • I2C FALLBACK     > Available only while I²C recovery is active; restores the fail-safe I²C address (0x61) after single confirmation. Recovery
                                               then continues using the restored address.
  ...
  • The SYSTEM MENU will time out after a preset period of inactivity, discarding any unsaved changes and returning to the main interface. The save/discard/cancel exit menu
    requires explicit user action. The SERVICE MENU and DIAGNOSTICS use a longer inactivity timeout before returning to the main interface.
  ...
  • During normal operation, I²C communication loss is indicated and retried automatically. System/Quick menus are closed and any unconfirmed frequency edit is cancelled,
    while the SERVICE MENU remains accessible through its normal UP+DOWN gesture. SERVICE MENU and DIAGNOSTICS stay browsable during recovery; I2C FALLBACK is then exposed
    as a recovery action, and a fixed "!" at the upper-right of every page indicates the active I²C fault. SYSTEM UPTIME continues to refresh, live FL/input fields show "-"
    and PLL lock time shows "<unknown>". A read-only interruption also invalidates the measured lock time until a new uninterrupted acquisition starts. Pending settings are
    still discarded for safety. After communication is restored, the PLL is fully reprogrammed only if a write may have failed or the TSA5511 POR flag indicates an
    unexpected reset; otherwise existing programming is retained and lock verification resumes as needed.
*/
...
// === SYSTEM CONSTANTS ===
    ...
    const char lcdTextLockTimeOverflow[] PROGMEM = ">9999 s";
    ...
    const unsigned long diagnosticsInterval = 50; // live diagnostics refresh interval
    const uint8_t diagnosticCountSaturation = 100; // shared saturation threshold for diagnostic counters
    static_assert(diagnosticCountSaturation == 100, "Update saturated diagnostic count text if threshold changes");
    ...
    // PLL settings
        ...
        const uint8_t defaultXTALFreqIndex = 1; // default PLL crystal frequency index
        ...
        const uint8_t defaultPLLAddrIndex = 1; // default I²C address index
        ...
        static_assert(PLL_PORT_NONE == 8, "Output-port indexing assumes P0..P7");
    ...
    const uint8_t defaultFreqBandIndex = FREQ_BAND_WORLD; // default VCO frequency band
    const uint8_t numFreqBands = sizeof(freqBands) / sizeof(freqBands[0]); // total number of selectable VCO frequency bands
    ...
    const uint8_t numUserMemorySlots = 6; // number of user memory slots per VCO frequency band and XTAL frequency
    ...
    // timing parameters
        ...
        const uint8_t millisecondsPerTenth = millisecondsPerSecond / 10; // milliseconds per tenth of a second
        ...
        const unsigned long lockTimeTenthsLimit = 100UL * millisecondsPerSecond; // tenths display threshold once locked
        const unsigned long lockingTimeTenthsLimit = 1000UL * millisecondsPerSecond; // tenths display threshold while locking
        const unsigned long lockTimeDisplayLimit = 10000UL * millisecondsPerSecond; // numeric lock-time display limit
        static_assert(lockTimeDisplayLimit == 10000UL * millisecondsPerSecond, "Update lock-time overflow text if display limit changes");

        // startup timing
        const unsigned long pllStartupDelay = 250; // TSA5511 power-up settling delay before first I²C access

        // UI display delays and intervals
        const unsigned long splashDelay = 2500; // startup splash duration; PLL initialization and monitoring run concurrently
        ...
        const unsigned long serviceMenuTimeout = 300000; // extended inactivity timeout in service/diagnostics mode
        ...
        const uint16_t pressAccelerationOffset = initialPressDelay * 7 / 10; // base factor expressed as an integer delay term
    ...
    static_assert(MENU_LEVEL_GENERAL_SETTINGS + 1 == MENU_LEVEL_SERVICE_MENU &&
        MENU_LEVEL_DIAGNOSTICS + 1 == MENU_LEVEL_QUICK_MENU,
        "Menu-level ordering assumptions broken");
...
// === RUNTIME STATE VARIABLES ===
    ...
    uint8_t i2cRecoveryCount = 0; // successfully completed I²C recoveries since controller startup (displayed with the saturation marker at the limit)
    ...
    uint8_t porCount = 0; // unexpected POR events observed since controller startup, excluding the initial POR baseline (displayed with the saturation marker at the limit)
    ...
    // startup splash
    unsigned long splashStartTime = 0; // startup splash start timestamp
    bool splashActive = false; // true while the startup splash suppresses normal UI input/output
    ...
    uint8_t freqBandIndex[numXTALFreqOptions]; // currently selected VCO frequency band index per XTAL frequency
    ...
    struct { // temporary system settings, committed only on SAVE CHANGES
        ...
        uint8_t freqBandIndex[numXTALFreqOptions];
    ...
    bool inputReleaseGuard = false; // suppress input across context transitions until all physical buttons are released
...
// === SYSTEM STATE UTILITIES ===
    ...
    // call regularly to maintain uptime across millis() rollovers
...
// === FREQUENCY UTILITIES ===
    ...
    uint8_t getStepSizeMultiplier() {
        const unsigned long vcoStep = getVCOFreqStep();
        ...
        uint8_t multiplier = (displayStepSize + vcoStep - 1) / vcoStep;
        while (vcoStep * multiplier % displayStepSize) multiplier++; // ensure each PLL step is exactly representable at current display precision
        return multiplier;
    }
    ...
    unsigned long validateFreq(unsigned long frequency, bool alignToStepSize = false, int8_t alignDirection = 0) {
        const unsigned long vcoStep = getVCOFreqStep();
        frequency = constrain(frequency, vcoStep, (unsigned long)PLL_DIVISOR_LIMIT * vcoStep); // constrain VCO frequency within valid PLL divisor range
        unsigned long step = vcoStep * (alignToStepSize ? getStepSizeMultiplier() : 1); // select step size (base PLL step size or visible step size)
...
void initialize() {
    ...
    inputReleaseGuard = true; // suppress all UI input until the splash has ended and buttons are released
    display(SPLASH_SCREEN);
    readSettings();
    delay(pllStartupDelay);
...
// restore safe default I²C address (0x61) during runtime recovery
void restoreI2CDefaults() {
    ...
    display(MENU_INTERFACE); // remain in SERVICE MENU while recovery continues on the restored address
    inputReleaseGuard = true;
}
...
unsigned long readLastBandFrequency(uint8_t bandIndex, uint8_t xtalIndex) {
...
void readPLLControl() {
    uint8_t val = EEPROM.read(EEPROM_PLL_CONTROL_ADDR); // read stored PLL control mode
    pllControlMode = (val <= VARICAP_DRIVE_OFF) ? val : defaultPLLControlMode; // validate and apply default if needed
}

void readOutputPorts() {
    uint8_t lockVal = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_LOCKED); // read locked index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank
    uint8_t unlockVal = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_UNLOCKED); // read unlocked index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank
    uint8_t rfVal = EEPROM.read(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_RF_DRIVE); // read RF drive index, PLL_PORT_NONE, or EEPROM_ERASED_BYTE if EEPROM is blank

    // apply default output port mapping in case of uninitialized/blank EEPROM
    if (lockVal == EEPROM_ERASED_BYTE && unlockVal == EEPROM_ERASED_BYTE && rfVal == EEPROM_ERASED_BYTE) {
    ...
    portIdxLock = isValidPLLPort(lockVal) ? lockVal : PLL_PORT_NONE; // locked index or 'none'
    portIdxUnlock = isValidPLLPort(unlockVal) ? unlockVal : PLL_PORT_NONE; // unlocked index or 'none'
    portIdxRF = isValidPLLPort(rfVal) ? rfVal : PLL_PORT_NONE; // RF drive index or 'none'
...
void handleBacklightControl() {
    ...
    unsigned long currentMillis = millis();
    const bool buttonActivity =
        !digitalRead(downButton) || !digitalRead(setButton) || !digitalRead(upButton);

    // restore brightness on any raw button activity
    if (buttonActivity) {
        if (currentBrightness != maxBrightness) {
            currentBrightness = maxBrightness;
            analogWrite(lcdBacklight, currentBrightness);
    ...
    if (!rfOutputEnabled) {
        ...
        if (!backlightOff && currentMillis - lastRFOutputStatusTime >= rfOutputStatusInterval) {
            ...
            display(MAIN_INTERFACE, false); // alternate only the status row; frequency/lock display remains unchanged
        }
        return;
    }
...
void handleMainInput() {
    ...
    if (inputReleaseGuard && !freqSetMode) { // let the frequency editor finish its active SET gesture
        servicePress = 0;
        if (!splashActive && !buttonDownState && !buttonSetState && !buttonUpState &&
            digitalRead(downButton) && digitalRead(setButton) && digitalRead(upButton)) {
    ...
    if (!freqSetMode && !backlightOff) {
        ...
        if (servicePress == SERVICE_PRESS_CHORD || servicePress == SERVICE_PRESS_CANCELLED) {
            if (servicePress == SERVICE_PRESS_CHORD && downHeld && upHeld && setReleased) {
                if (currentMillis - freqSetInactivityTimer >= serviceMenuHoldDelay) {
                    ...
                    inputReleaseGuard = true; // keep the new menu context inert until the entry transition and opening-button release are complete
        ...
        if (setReleased && (downHeld != upHeld)) {
            ...
            if (!servicePress || currentMillis - freqSetInactivityTimer <= serviceChordWindow) {
                ...
                freqSetInactivityTimer = currentMillis; // the chord hold starts when both buttons are actually down
                return;
            }
...
void handleMenu() {
    ...
    // keep context transitions input-atomic until all physical buttons have been released
    if (inputReleaseGuard) {
        // require both the loop snapshot and live pins released so a SET release cannot be erased between readButtons() and here
        if (!splashActive && !buttonDownState && !buttonSetState && !buttonUpState &&
            digitalRead(downButton) && digitalRead(setButton) && digitalRead(upButton)) {
            inputReleaseGuard = false;
            buttonDownPressed = buttonUpPressed = false;
    ...
    // discard changes and exit menu on timeout if inactive
    if (menuMode && !menuExitConfirmMode &&
        ...
        if (!buttonSetState && clickStartTime != 0) {
            ...
            if (clickDuration < setClickInterval) {
                ...
                if (validDoubleClick) { // valid double-click detected: enter settings menu
                    ...
                    outputPortsEditPhase = OUTPUT_PORT_PHASE_LOCKED;
                    display(MENU_ENTRY_TITLE);
                    inputReleaseGuard = true; // accept menu input only after the opening gesture is fully released
                    menuInactivityTimer = millis();
                    return;
                }
            }
    ...
    if (menuLevel == MENU_LEVEL_SERVICE_MENU) {
        if (menuEditMode) {
            ...
            // I2C FALLBACK confirms directly; FACTORY RESET retains its additional confirmation step
        ...
        if (buttonDownState && !buttonDownPressed) {
            ...
            if (!i2cRecoveryState && menuIndex == SERVICE_MENU_I2C_FALLBACK) menuIndex--;
            display(MENU_INTERFACE);
    ...
    if (menuLevel == MENU_LEVEL_DIAGNOSTICS) {
        ...
        if ((!i2cRecoveryState || menuIndex == DIAGNOSTICS_MENU_UPTIME) && menuIndex != DIAGNOSTICS_MENU_EXIT &&
            currentMillis - lastDiagnosticsRefresh >= diagnosticsInterval) {
    ...
    if (menuExitConfirmMode) {
        ...
        if (setShortAction) {
            if (menuIndex == EXIT_MENU_SAVE_CHANGES) { // save changes
                ...
                readLastFrequencyBand();
                if (pllConfigChanged || targetFreq != currentFreq) configurePLL(true); // apply PLL configuration only when required
                restoreSettings();
        ...
        if (menuLevel == MENU_LEVEL_VCO_SETTINGS) {
            switch (menuIndex) {
                ...
                case VCO_MENU_PRECISION:
                    handleButtonRepeat(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (menuSettings.numDecimals > 0) {
                            menuSettings.numDecimals--;
                            display(MENU_INTERFACE);
                        }
                    });
                    handleButtonRepeat(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (menuSettings.numDecimals < PLL_XTAL_MAX_DECIMALS[menuSettings.xtalFreqIndex]) {
                            menuSettings.numDecimals++;
                            display(MENU_INTERFACE);
                        }
                    });
            ...
            switch (menuIndex) {
                case PLL_MENU_I2C_ADDRESS:
                    handleButtonRepeat(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (menuSettings.pllAddrIndex > 0) {
                            menuSettings.pllAddrIndex--;
                            display(MENU_INTERFACE);
                        }
                    });
                    handleButtonRepeat(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (menuSettings.pllAddrIndex + 1 < numPLLAddresses) {
                            menuSettings.pllAddrIndex++;
                            display(MENU_INTERFACE);
                        }
                    });
                ...
                case PLL_MENU_PLL_CONTROL:
                    handleButtonRepeat(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (menuSettings.pllControlMode == VARICAP_DRIVE_OFF) {
                            menuSettings.pllControlMode = CP_LOW;
                            display(MENU_INTERFACE);
                        } else if (menuSettings.pllControlMode == CP_LOW) {
                            menuSettings.pllControlMode = CP_HIGH;
                            display(MENU_INTERFACE);
                        }
                    });
                    handleButtonRepeat(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (menuSettings.pllControlMode == CP_HIGH) {
                            menuSettings.pllControlMode = CP_LOW;
                            display(MENU_INTERFACE);
                        } else if (menuSettings.pllControlMode == CP_LOW) {
                            menuSettings.pllControlMode = VARICAP_DRIVE_OFF;
                            display(MENU_INTERFACE);
                        }
                    });
                    break;
            ...
            switch (menuIndex) {
                case GENERAL_MENU_BACKLIGHT_DIMMER:
                    handleButtonRepeat(buttonDownState, buttonDownPressed, -1, [](int8_t) {
                        if (menuSettings.backlightDimActive) {
...
void applyStationNameEdit(bool editCharacter, int8_t direction, bool finishEdit) {
    if (editCharacter) {
        ...
        if (finishEdit || editPosition >= maxNameLength - 1) { // finish station name edit on SET long-press or when last character is confirmed
            ...
            inputReleaseGuard = true; // wait for physical button release before accepting new menu input
...
void handleButtonRepeat(bool buttonState, bool& buttonPressed, int8_t direction, void (*action)(int8_t)) {
    ...
    if (buttonState) {
        ...
        if (!buttonPressed) {
            ...
            if (!fastPressInterval) fastPressInterval = 1; // retain a finite timing limit after exceptionally long holds
...
void configurePLL(bool force) {
    ...
    data[2] = PLL_BYTE4_BASE // use high charge-pump current during acquisition, unless varicap drive is disabled
        | ((pllControlMode == VARICAP_DRIVE_OFF) ? PLL_BYTE4_OS : PLL_BYTE4_CP);
...
void monitorPLL() {
    ...
    if (splashActive && currentMillis - splashStartTime >= splashDelay) {
        splashActive = false;
        if (i2cRecoveryState) display(I2C_ERROR);
        else {
            display(MAIN_INTERFACE);
    ...
    if (!attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
        ...
        if (!i2cRecoveryState) {
            ...
            if (recoveryMenuActive) display(MENU_INTERFACE); // show the SERVICE/DIAGNOSTICS fault state immediately
            else display(I2C_ERROR);
        }
        return;
    }

    currentMillis = millis(); // timestamp successful status read completion for precise PLL acquisition timing
    retryCount = 0;
    ...
    if (i2cRecoveryState || (statusByte & PLL_BYTE1_POR)) {
        ...
        if (recoveryState) display(LCD_INITIALIZE); // resynchronize LCD in case the fault disturbed its controller state
        digitalWrite(lockIndicator, pllLock ? HIGH : LOW);
        if (recoveryMenuActive) {
            if (menuLevel == MENU_LEVEL_SERVICE_MENU && menuIndex == SERVICE_MENU_I2C_FALLBACK) {
                menuIndex = SERVICE_MENU_DIAGNOSTICS;
                menuEditMode = false;
                serviceActionConfirm = factoryResetFinalConfirm = false;
            }
            ...
            inputReleaseGuard = buttonSetState;
            readSetGesture(true); // suppress any menu key still held as recovery completes
            menuInactivityTimer = currentMillis;
            display(MENU_INTERFACE);
        } else {
            inputReleaseGuard = true; // suppress any main-screen button still held as recovery completes
            display(MAIN_INTERFACE);
            display(PLL_LOCK_STATUS);
        }
        return;
...
void storeBandFrequency(uint8_t bandIndex, uint8_t xtalIndex, unsigned long frequency) {
...
void storeOutputPorts() {
    auto encodePort = [](uint8_t portIndex) { return isValidPLLPort(portIndex) ? portIndex : PLL_PORT_NONE; }; // encode port index into storage format or 'none'
    EEPROM.update(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_LOCKED, encodePort(portIdxLock));
    EEPROM.update(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_UNLOCKED, encodePort(portIdxUnlock));
    EEPROM.update(EEPROM_OUTPUT_PORTS_ADDR + OUTPUT_PORT_PHASE_RF_DRIVE, encodePort(portIdxRF));
}
...
uint16_t getUserMemoryAddress(uint8_t slotIndex) {
...
bool readUserMemoryFrequency(uint8_t slotIndex, unsigned long& frequency) {
...
void storeUserMemoryFrequency(uint8_t slotIndex) {
    if (slotIndex >= numUserMemorySlots) return;

    EEPROM.put(getUserMemoryAddress(slotIndex), currentFreq);
}

void clearUserMemoryFrequency(uint8_t slotIndex) {
    if (slotIndex >= numUserMemorySlots) return;
...
void display(uint8_t mode, bool refresh) {
    ...
    if (splashActive && mode != SPLASH_SCREEN && mode != LCD_INITIALIZE) return;
    if (i2cRecoveryState && mode != I2C_ERROR && mode != FACTORY_RESET_STATUS &&
    ...
    if (mode != MENU_ENTRY_TITLE) menuEntryAnimStep = 0; // substantive display updates supersede a running menu-entry animation
    ...
    // format right-aligned VCO frequency with optional prefix, fixed MHz suffix and dynamic decimal precision
    auto printFreq = [&](uint8_t row, unsigned long frequency, const __FlashStringHelper* prefix) {
        ...
        uint8_t col = lcdColumns - (freqDisplayWidth + sizeof(lcdSuffixMHz) - 1); // compute starting column for fixed-width frequency field
        if (prefix) {
            lcd.setCursor(0, row);
            lcd.print(prefix);
        }
    ...
    auto buildLine = [](char* line, const char* text, bool centered) {
        ...
        if (length > lcdColumns) length = lcdColumns;
        uint8_t col = centered ? (lcdColumns - length) / 2 : 0;
        memcpy_P(line + col, text, length);
    };
    ...
    switch(mode) {
        ...
        case MAIN_INTERFACE:
            if (refresh) printFreq(0, targetFreq, nullptr);
            lcd.setCursor(0, 1);
            if (!rfOutputEnabled && ((millis() / rfOutputStatusInterval) % 2)) {
                clearLine(1); // clear row before showing the RF drive override status
                lcd.setCursor(0, 1);
        ...
        case SET_FREQUENCY_INTERFACE:
            ...
            printFreq(1, targetFreq, nullptr);
            break;
        ...
        case MENU_INTERFACE:
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                ...
                switch (userMemoryAction) {
                    case USER_MENU_RECALL_MEMORY: lcd.print(FPSTR(lcdTextRecallMemory)); break;
                    case USER_MENU_STORE_MEMORY: printFreq(0, currentFreq, F("SAVE ")); break;
                    case USER_MENU_CLEAR_MEMORY: lcd.print(FPSTR(lcdTextClearMemory)); break;
                }
                lcd.setCursor(0, 1);
                if (menuIndex == numUserMemorySlots) {
                    ...
                    if (readUserMemoryFrequency(menuIndex, memoryFreq)) {
                        ...
                        uint8_t valueLength = strlen(buffer) + sizeof(lcdSuffixMHz) - 1;
                ...
                switch (menuIndex) {
                    ...
                    case SERVICE_MENU_FACTORY_RESET:
                        lcd.print(factoryResetFinalConfirm ? F("CLEAR DATA?") : FPSTR(lcdTextFactoryReset));
                        lcd.setCursor(0, 1);
                        if (menuEditMode) {
                            lcd.print(FPSTR(lcdTextMenuCursor));
                            lcd.print(serviceActionConfirm ? FPSTR(lcdTextYes) : FPSTR(lcdTextNo));
                ...
                static uint8_t displayedDiagnosticsByte = 0, displayedDiagnosticsAux = 0;
                static bool displayedDiagnosticsValid = false;
                switch (menuIndex) {
                    case DIAGNOSTICS_MENU_STATUS: {
                        bool statusAvailable = pllStatusValid && !i2cRecoveryState;
                        ...
                        if (!refresh && statusAvailable == displayedDiagnosticsValid && (!statusAvailable || fl == (displayedDiagnosticsByte != 0)) &&
                            porCount == displayedDiagnosticsAux) break;
                        ...
                        displayedDiagnosticsAux = porCount;
                        break;
                    }
                    case DIAGNOSTICS_MENU_INPUTS: {
                        bool inputsAvailable = pllStatusValid && !i2cRecoveryState;
                        uint8_t inputs = pllStatusByte;
                        byte portCommand = pllPortByte;
                        if (!refresh && inputsAvailable == displayedDiagnosticsValid &&
                            (!inputsAvailable || (inputs == displayedDiagnosticsByte && portCommand == displayedDiagnosticsAux))) break;
                        if (refresh) lcd.print(F("P7 P5 P4 P6"));
                        lcd.setCursor(0, 1);
                        if (!inputsAvailable) lcd.print(F("-  -  -  ---"));
                        else {
                            // TSA5511 status bits 5..3 = P7/P5/P4; bits 2..0 = P6 ADC
                            auto printDigitalInput = [&](uint8_t port, uint8_t bit) {
                                if (portCommand & _BV(port)) lcd.print('-');
                                else lcd.print((inputs >> bit) & 1);
                                lcd.print(F("  "));
                            };

                            printDigitalInput(7, 5);
                            printDigitalInput(5, 4);
                            printDigitalInput(4, 3);

                            if (portCommand & _BV(6)) lcd.print(F("---"));
                            else for (int8_t bit = 2; bit >= 0; bit--) lcd.print((inputs >> bit) & 1);
                        }
                        displayedDiagnosticsByte = inputs;
                        displayedDiagnosticsAux = portCommand;
                        displayedDiagnosticsValid = inputsAvailable;
                        break;
                    }
                    case DIAGNOSTICS_MENU_LOCK_TIME: {
                        static bool displayedPending = false;
                        static char displayedTime[sizeof(lcdTextLockTimeOverflow)] = "";
                        ...
                        // live refresh only rewrites the lock-time field when its visible value changes
                        ...
                        char buffer[sizeof(lcdTextLockTimeOverflow)];
                        ...
                        if (elapsed < lockTimeDisplayLimit) {
                            ...
                            *p++ = 's';
                            *p = '\0';
                        } else memcpy_P(buffer, lcdTextLockTimeOverflow, sizeof(lcdTextLockTimeOverflow));
                    ...
                    case DIAGNOSTICS_MENU_I2C_RECOVERIES: {
                        if (!refresh && i2cRecoveryCount == displayedDiagnosticsAux) break;
                        if (refresh) {
                            lcd.print(F("I2C RECOVERIES"));
                            lcd.setCursor(0, 1);
                            lcd.print(F("count: "));
                        } else {
                            rightAlign(1, sizeof("count: ") - 1, sizeof(lcdTextCountSaturated) - 1, 0);
                            lcd.setCursor(sizeof("count: ") - 1, 1);
                        }
                        if (i2cRecoveryCount < diagnosticCountSaturation) lcd.print(i2cRecoveryCount);
                        else lcd.print(FPSTR(lcdTextCountSaturated));
                        displayedDiagnosticsAux = i2cRecoveryCount;
                        break;
                    }
        ...
        case LCD_INITIALIZE:
            ...
            if (splashActive) display(SPLASH_SCREEN);
            break;
    ...
    if ((menuLevel == MENU_LEVEL_SERVICE_MENU || menuLevel == MENU_LEVEL_DIAGNOSTICS) && i2cRecoveryState) printStatusMarker('!');
}
