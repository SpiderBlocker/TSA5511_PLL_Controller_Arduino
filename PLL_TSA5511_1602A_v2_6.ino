...
void readChargePump() {
    uint8_t v = EEPROM.read(EEPROM_CP_ADDR); // no check for invalid stored value required; any non-zero value reads as true
    cpMode = (v <= CP_DISABLED) ? v : CP_HIGH; // fail-safe default = high
}
    ...
    if (menuExitConfirmMode) {
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuIndex == 0) { // save changes
                ...
                storeDimmerStatus();
                readLastFrequencyBand();
                configurePLL(); // apply updated XTAL, charge pump and output ports settings
                restoreSettings();
