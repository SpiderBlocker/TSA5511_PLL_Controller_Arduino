...
USAGE
  ...
  • During normal operation, I²C communication loss is indicated and retried automatically. System/Quick menus are closed and any unconfirmed frequency edit is cancelled,
    ...
    data pages, live FL/input fields show "-" and PLL lock time shows "<unknown>" (RETURN TO MENU remains unmarked). A read-only interruption also invalidates the measured
    lock time until a new uninterrupted acquisition starts. Pending settings are still discarded for safety. After communication is restored, the PLL is fully reprogrammed
    only if a write may have failed or the TSA5511 POR flag indicates an unexpected reset; otherwise existing programming is retained and lock verification resumes as needed.
*/
...
// === RUNTIME STATE VARIABLES ===
    ...
    // PLL settings
    unsigned long lockTime = 0; // acquisition start timestamp while pending; last successful lock duration otherwise; ~0UL if unknown
...
void handleMenu() {
    ...
    if (menuMode) display(MENU_ENTRY_TITLE, false); // advance a running menu-entry animation without blocking menu/PLL handling
    ...
    if (menuExitConfirmMode) {
        ...
        if (setShortAction) {
            if (menuIndex == EXIT_MENU_SAVE_CHANGES) { // save changes
                ...
                if (tempPllAddrIndex != pllAddrIndex) {
        ...
        if (menuLevel == MENU_LEVEL_VCO_SETTINGS) {
            switch (menuIndex) {
                ...
                case VCO_MENU_PRECISION:
                    ...
                    handleButtonRepeat(buttonUpState, buttonUpPressed, 1, [](int8_t) {
                        if (numDecimals < getMaxNumDecimals()) numDecimals++;
                        display(MENU_INTERFACE);
                    });
                    break;
            }
...
void configurePLL(bool force) {
    ...
    if (attemptI2C(false, PLL_ADDRESSES[pllAddrIndex], data, 4)) {
        lockTime = millis();
        if (lockTime == ~0UL) lockTime--; // reserve sentinel used for invalid lock timing across millis rollover
        pllPortByte = data[3];
...
void monitorPLL() {
    ...
    if (startupServiceMode) {
        ...
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
            ...
            if (retryCount && menuIndex != DIAGNOSTICS_MENU_EXIT) display(I2C_STATUS_MARKER, false); // clear stale-data marker immediately after communication recovers
            retryCount = 0;
            pllStatusByte = statusByte;
            pllStatusValid = true;
        } else {
            if (retryCount < i2cMaxRetries) retryCount++;
            if (menuIndex != DIAGNOSTICS_MENU_EXIT) display(I2C_STATUS_MARKER);
        }
        return;
    }
    ...
    if (!attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
        ...
        if (!i2cRecoveryState) {
            ...
            lockTime = ~0UL; // any read interruption invalidates the measured acquisition timing
...
void display(uint8_t mode, bool refresh) {
    ...
    static uint8_t menuEntryAnimStep = 0;
    static bool stationNameCursorVisible = false;
    static bool stationNameCursorActive = false;

    if (i2cRecoveryState && mode != I2C_ERROR && mode != I2C_STATUS_MARKER &&
    ...
    switch(mode) {
        ...
        case MENU_INTERFACE:
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                ...
                switch (menuIndex) {
                    case DIAGNOSTICS_MENU_STATUS:
                        ...
                        lcd.setCursor(0, 1);
                        lcd.print(F("FL:"));
                        if (pllStatusValid && !i2cRecoveryState && !(startupServiceMode && retryCount >= i2cMaxRetries)) lcd.print((pllStatusByte & PLL_BYTE1_FL) ? 1 : 0);
                        else lcd.print(F("-"));
                        lcd.print(F(" POR cnt:"));
                    ...
                    case DIAGNOSTICS_MENU_INPUTS:
                        lcd.print(F("P7 P5 P4 P6"));
                        lcd.setCursor(0, 1);
                        if (!pllStatusValid || i2cRecoveryState || (startupServiceMode && retryCount >= i2cMaxRetries)) {
                            lcd.print(F("-  -  -  ---"));
                            break;
                        }
                    ...
                    case DIAGNOSTICS_MENU_LOCK_TIME: {
                        ...
                        if (i2cRecoveryState || lockTime == ~0UL) {
                            lcd.print(F("PLL LOCK TIME"));
                            lcd.setCursor(0, 1);
                            lcd.print(F("<unknown>"));
                            break;
            ...
            if (i2cRecoveryState && menuLevel == MENU_LEVEL_DIAGNOSTICS && menuIndex != DIAGNOSTICS_MENU_EXIT) {
        ...
        case MENU_ENTRY_TITLE: {
            ...
            const char* targetText = menuLevel == MENU_LEVEL_SERVICE_MENU ? PSTR("DIAGNOSTICS") :
