...
USAGE
  ...
  • Hold UP and DOWN together for 1 second from the main interface to open the SERVICE MENU. A brief chord-acquisition window delays the first individual UP/DOWN action
    ...
    ■ SERVICE MENU     => • DIAGNOSTICS      > Shows live TSA5511 lock/input status, live/completed PLL lock acquisition timing, PLL divisor and unexpected POR count.
                                               ...
                                               the SERVICE MENU. Before normal PLL initialization, the menu uses read-only I²C probing; live TSA5511 values remain '-'
                                               until the first successful status read. Failed reads mark all startup SERVICE MENU screens with '!' immediately, while
                                               sustained startup failures replace stale live values with '-'. The first successful read establishes the expected power-on POR
                                               baseline; later POR events and sustained I²C losses that subsequently recover are counted. Lock timing, PLL divisor and
                                               output-port command data are shown as not initialized.
                                               INPUTS shows the raw digital states of P7, P5 and P4 and the three-bit ADC code of P6. During normal operation, actively
                                               driven ports are masked as '-'/'---'; high-impedance ports show their external input levels. Startup SERVICE MENU keeps the
                                               raw status values because the previous TSA5511 output-port state may be unknown.
                          ...
                          • I2C FALLBACK     > Available only when the SERVICE MENU is entered during startup; restores the fail-safe I²C address (0x61) after single
                                               confirmation and returns to the startup SERVICE MENU. An active startup I²C fault remains indicated during fallback;
                                               normal initialization resumes only after explicitly leaving the menu.
  ...
  • During normal operation, I²C communication loss is indicated and retried automatically. System/Quick menus are closed and any unconfirmed frequency edit is cancelled,
    while the SERVICE MENU remains accessible through its normal UP+DOWN gesture. SERVICE MENU and DIAGNOSTICS stay browsable during recovery; a fixed "!" at the upper-right
    of every page indicates an active I²C fault, while SYSTEM UPTIME continues to refresh. Live FL/input fields show "-" and PLL lock time shows "<unknown>". A read-only
    interruption also invalidates the measured lock time until a new uninterrupted acquisition starts. Pending settings are still discarded for safety. After communication
    is restored, the PLL is fully reprogrammed only if a write may have failed or the TSA5511 POR flag indicates an unexpected reset; otherwise existing programming is
    retained and lock verification resumes as needed.
*/
...
// === SYSTEM CONSTANTS ===
    ...
    const unsigned long diagnosticsInterval = 50; // live diagnostics refresh and startup SERVICE MENU I²C probe interval
...
void handleStartupServiceMenu() {
    ...
    while (startupServiceMode) {
        ...
        readButtons();
        handleMenu();
        if (startupServiceMode) monitorPLL(); // read-only status probing throughout the startup SERVICE MENU
    }
    retryCount = 0; // do not carry startup SERVICE MENU retries into normal initialization
...
void handleMenu() {
    ...
    auto returnToMainInterface = []() {
        if (startupServiceMode) {
            ...
            inputReleaseGuard = buttonSetState; // ignore a still-held SET until release
            return;
        }
        menuMode = false;
        menuEditMode = false;
...
void monitorPLL() {
    ...
    // startup SERVICE MENU probes raw status without entering normal recovery or programming the PLL
    if (startupServiceMode) {
        if (retryCount) blinkLed(errIndicator, errBlinkRate);
        ...
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1, true)) {
            // count restored communication after a sustained startup SERVICE MENU fault
            ...
            if (retryCount) {
                retryCount = 0;
                display(I2C_STATUS_MARKER, false); // clear the fault marker immediately after communication recovers
            }
            digitalWrite(errIndicator, LOW);
...
void display(uint8_t mode, bool refresh) {
    ...
    if (mode != MENU_ENTRY_TITLE && mode != I2C_STATUS_MARKER) menuEntryAnimStep = 0; // substantive display updates supersede a running menu-entry animation
            ...
            if (menuLevel == MENU_LEVEL_QUICK_MENU) {
                ...
                switch (menuIndex) {
                    ...
                    case DIAGNOSTICS_MENU_INPUTS: {
                        ...
                        byte portCommand = startupServiceMode ? 0 : pllPortByte;
    ...
    if (((menuLevel == MENU_LEVEL_SERVICE_MENU || menuLevel == MENU_LEVEL_DIAGNOSTICS) && i2cRecoveryState) ||
        (startupServiceMode && retryCount)) printStatusMarker('!');
}
