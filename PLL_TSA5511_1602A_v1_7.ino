...
const unsigned long charScrollInterval = 300; // interval between character scroll steps
...
bool pllWatchdog = false; // true if new PLL divisor value is pending
...
void readButtons() {
    ...
    // invalidate button states on any combination of DOWN/SET/UP
    if ((buttonDownState + buttonSetState + buttonUpState) > 1) {
        buttonDownState = buttonSetState = buttonUpState = false;
        lastMultiButtonPressTime = currentMillis;
        return;
    }

    // suppress actions shortly after release of buttons
    if (currentMillis - lastMultiButtonPressTime < buttonTimingTolerance) {
        buttonDownState = buttonSetState = buttonUpState = false;
    }
}
...
void readFrequency() {
    if (!freqSetMode) {
        ...
        if (storedFreq < lowerFreq || storedFreq > upperFreq) {
            targetFreq = lowerFreq; // default initial frequency
        } else {
            targetFreq = round(storedFreq / PLL_REF_FREQ) * PLL_REF_FREQ; // round to closest multiple of PLL_REF_FREQ
        }
    }
}
...
void handleBacklightControl() {
    ...
    if (stationNameEditMode || freqSetMode || !pllLock) {
    ...
    if (!backlightControlActive) {
        while (!digitalRead(setButton) && digitalRead(downButton) && digitalRead(upButton)); // skip if multiple buttons are pressed
    ...
    if (buttonSetState) {
        ...
        if (currentMillis - lastSetButtonClickTime >= buttonTimingTolerance && currentMillis - lastSetButtonClickTime < doubleClickThreshold) {
            setButtonClickCount++;
        } else {
...
void handleStationNameEditor() {
    if (stationNameEditMode) {
        // select character
        auto nameChange = [](int8_t direction) { stationNameEditorAction(true, direction); };
        ...
        // confirm selection
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t direction) { stationNameEditorAction(false, direction); });
    } else {
        if (!initialized) { initialize(false); } // finalize initialization
    }
}

void stationNameEditorAction(bool nameChange, int8_t direction) {
    if (nameChange) {
        // UP/DOWN action
        uint8_t charRange = 127 - 32 + 1; // allowed ASCII character range
        stationName[stationNameEditPos] = (stationName[stationNameEditPos] - 32 + direction + charRange) % charRange + 32;
        display(STATION_NAME_EDITOR);
    } else {
        // SET action
        if (stationNameEditPos >= maxNameLength - 1) { // store station name when last character has been confirmed
            storeStationName();
            stationNameEditMode = false;
        } else {
            stationNameEditPos++; // move to next cursor position
            display(STATION_NAME_EDITOR);
        }
    }
}
...
void handleFrequencyChange() {
    ...
    if (initialized && !dimmerSetMode && !stationNameEditMode) {
        // change frequency
        auto freqChange = [](int8_t direction) { frequencyChangeAction(true, &targetFreq, direction); };
        ...
        if (freqSetMode && buttonSetState) {
            frequencyChangeAction(false, &targetFreq, 0);
            timedOut = true;
...
void frequencyChangeAction(bool freqChange, long* targetFreq, int8_t direction) {
    if (freqChange) {
        // UP/DOWN action
        if (freqSetMode) { *targetFreq += (direction * freqStep); }
...
void handleButtonInput(bool buttonState, bool& buttonPressed, int8_t direction, void (*action)(int8_t)) {
    ...
    if (buttonState) {
        // immediate action on button press
        if (!buttonPressed) {
        ...
        // action when holding button
            ...
            if (totalPressTime >= initialPressDelay &&
                ...
                lastPressTime = currentMillis;
                action(direction);
                if (stationNameEditMode) delay(charScrollInterval); // fixed scroll speed in station name editor
            }
        }
    } else {
        buttonPressed = false; // reset on button release to detect next press
    }
}

void configurePLL() {
    if (targetFreq == currentFreq) return;
    long divisor = (targetFreq / PLL_REF_FREQ); // calculate divisor
...
void display(uint8_t mode) {
    ...
    auto printFreq = []() {
        lcd.print(targetFreq < 10000000 ? "   " : (targetFreq < 100000000 ? "  " : (targetFreq < 1000000000 ? " " : "")));
        lcd.print(targetFreq / 1000000.0, 2);
        lcd.print(" MHz");
    };
    ...
    switch(mode) {
        ...
        case STATION_NAME_EDITOR:
            ...
            lcd.setCursor(stationNameEditPos, 1);
            lcd.cursor();
            break;
