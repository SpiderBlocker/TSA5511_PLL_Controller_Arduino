...
USAGE
  ...
  • PLL lock is verified after programming. To prevent false unlock indications caused by FM modulation, continuous lock polling is intentionally avoided after lock
    has been detected. The TSA5511 power-on reset flag is monitored and triggers automatic PLL reprogramming after an unexpected IC reset.
...
void checkPLL() {
    ...
    byte statusByte;
    if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &statusByte, 1)) {
        if (statusByte & PLL_BYTE1_POR) {
            configurePLL(true); // restore PLL configuration after an unexpected TSA5511 reset
            return;
        }
...
void checkI2C() {
    ...
    if (currentMillis - lastI2cCheckTime >= i2cHealthCheckInterval) {
        ...
        byte readByte;
        if (attemptI2C(true, PLL_ADDRESSES[pllAddrIndex], &readByte, 1) && (readByte & PLL_BYTE1_POR)) {
            configurePLL(true); // restore PLL configuration after an unexpected TSA5511 reset
        }
    }
}
