...
// === PLL OUTPUT PORT UTILITIES ===
    ...
    bool isMappedToOtherOutput(uint8_t portIndex) {
        ...
        return (outputPortsEditPhase != 0 && portIndex == portIdxLock)
            || (outputPortsEditPhase != 1 && portIndex == portIdxUnlock)
            || (outputPortsEditPhase != 2 && portIndex == portIdxRF);
    }

    void adjustOutputPort(int8_t direction) {
        uint8_t* portIndex = (outputPortsEditPhase == 0) ? &portIdxLock : ((outputPortsEditPhase == 1) ? &portIdxUnlock : &portIdxRF);
        ...
        } while (isMappedToOtherOutput(*portIndex));
    }
                ...
                switch (menuIndex) {
                    case 0:
                        ...
                        if (menuEditMode) {
                            ...
                            lcd.print(F("> "));
                            lcd.print(low / 1000000UL);
                            lcd.print(F("-"));
                            lcd.print(high / 1000000UL);
                            lcd.print(F(" MHz"));
                        } else {
