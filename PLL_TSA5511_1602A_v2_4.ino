...
// === RUNTIME STATE VARIABLES ===
    ...
    uint8_t outputPortsEditPhase = 0; // 0 = "PLL lock", 1 = "PLL unlock", 2 = "RF drive"
    ...
    if (menuExitConfirmMode) {
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuIndex == 0) { // save changes
                ...
                storeDimmerStatus();
                readLastFrequencyBand();
                configurePLL(); // apply updated charge pump and XTAL settings
                restoreSettings();
    ...
    if (!menuEditMode) {
        ...
        handleButtonInput(buttonSetState, buttonSetPressed, 0, [](int8_t) {
            if (menuLevel == 2 && menuIndex == 3) {
                // advance edit phase: lock -> unlock -> RF drive -> exit edit mode
